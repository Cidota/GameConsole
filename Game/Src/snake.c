...
void render_snake_UI()
{
	...
	{
		if (!isReset)
		{ // initial background for score and game over
			drawRectangle(0, 0, 160, getScreenHeight() - getImageHeight(SNAKE_TEXT_PAB), BLACK);
		}
		else
		{ // draw game over if we are in a reset state
	...
	{ // Draw high scores if we are in game
...
void init_snake_game(int x, int y)
{
	...
	if (snake_head == NULL || snake_tail == NULL || map_size == NULL || apple == NULL || next_position == NULL)
	{
...
void update_snake_game()
{
	...
	if (!isGameRunning)
	{
		...
		{
			if (isReset)
			{ // if we need to reset then check if the reset timer has completed
				if (getTimeMs() - timeReset < 2000)
				{
					return;
				}
				else
				{ // if it has then render the apple and snake now
					init_render();
					isReset = false;
				}
			}
	...
	if (next_position->x < 0 || next_position->x >= map_size->x || next_position->y < 0 || next_position->y >= map_size->y || (map[indexNextPos]->type == SNAKE && indexNextPos != getIndex(snake_tail)))
	{
		reset_snake_game();
		return;
	}
